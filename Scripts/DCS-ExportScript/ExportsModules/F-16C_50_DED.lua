...
function mergeString(original_string, new_data, location)
...
function buildDEDLine(line)
-- Get Layout Information for line being built
	local DEDLayoutLine = DEDLayout[line]
-- Get Exported DED Objects
	local DED_fields = parse_indication(6)  -- Get Indicator Dict
	local layout
	local label
	local value

-- Base Output String
	local dataLine ="                         "  -- Preset data line with 25 Chars.
	local dataLine_Inv ="                         "  -- Preset data line with 25 Chars.
	local strInv=""
...
--Get layout data associated with current key
		layout = DEDLayoutLine[label:gsub("_inv","",1):gsub("_lhs","_both",1)]  --Get Layout info from Label, ignore INV, turn Asterisks "lhs" to "Both" with 2nd Value.
		if layout ~= nil then
--If layout value 6 is present then use this value to override the value returned from DCS
			if layout[6] ~= nil then   -- Some item has preset value from DED Layout dict.
				value = layout[6]
			else
...
-- Adding format/inv string.
			-- Asterisks
			if label:find("_lhs") ~= nil then
				layout = DEDLayoutLine[label:gsub("_lhs","_both",1)]
				if layout ~=nil then
					dataLine_Inv = mergeString(dataLine_Inv, "*", layout[1])
					dataLine_Inv = mergeString(dataLine_Inv, "*", layout[3])
				end
			end

			-- Scratchpad
			if label:find("_inv") ~= nil then
				layout = DEDLayoutLine[label:gsub("_inv","",1)]
				if layout ~=nil then
					strInv=""
					for i=1,layout[2],1 do
						strInv = strInv .."*"
					end
					dataLine_Inv = mergeString(dataLine_Inv, strInv, layout[1])
				end
			end

			-- List Item Number
			if (label:find("List Item") ~=nil or label:find("Misc Item") ~=nil) and label:find("Number") ~=nil then
				layout = DEDLayoutLine[label]
				if layout ~=nil then
					dataLine_Inv = mergeString(dataLine_Inv, "*", layout[1])
				end
			end
	...
	rdataline={}
	rdataline["dat"]=dataLine:gsub("a","^")
	rdataline["inv"]=dataLine_Inv


    return rdataline --{dataLine,dataLine_Inv}
end


function buildDEDLine_li(li,line)
	-- Get Layout Information for line being built
	local DEDLayoutLine = DEDLayout[line]
	-- Get Exported DED Objects
	--local DED_fields = parse_indication(6)  -- Get Indicator Dict
